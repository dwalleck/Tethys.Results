...
namespace Tethys.Results
{
    ...
    public sealed class Result<T> : IResult, IEquatable<Result<T>>
    {
        ...
        public static Result<IEnumerable<T>> Combine(IEnumerable<Result<T>> results)
        {
            ...
            // Collect all data and failures in a single pass
            var successData = new List<T>();
            ...
            {
                return Result<IEnumerable<T>>.Ok(successData, "All operations completed successfully");
            }
        ...
        public static async Task<Result<T>> TryAsync(Func<Task<T>> asyncFunc)
        {
            ...
            try
            {
                var value = await asyncFunc().ConfigureAwait(false);
                return Ok(value);
            }
            catch (Exception ex)
            {
